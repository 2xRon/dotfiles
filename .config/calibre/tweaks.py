...
# The algorithm used to assign a book added to an existing series a series number. New series numbers assigned using this tweak are always integer values, except if a constant non-integer is specified. Possible values are:
...
# Should the completion separator be append to the end of the completed text to automatically begin a new completion operation for authors. Can be either True or False
...
#  When this tweak is changed, the author_sort values stored with each author must be recomputed by right-clicking on an author in the left-hand tags pane, selecting 'manage authors', and pressing 'Recalculate all author sort values'. The author name suffixes are words that are ignored when they occur at the end of an author name. The case of the suffix is ignored and trailing periods are automatically handled. The same is true for prefixes. The author name copy words are a set of words which if they occur in an author name cause the automatically generated author sort string to be identical to the author name. This means that the sort for a string like Acme Inc. will be Acme Inc. instead of Inc., Acme
...
# By default, calibre splits a string containing multiple author names on ampersands and the words "and" and "with". You can customize the splitting by changing the regular expression below. Strings are split on whatever the specified regular expression matches, in addition to ampersands. Default: r'(?i),?\s+(and|with)\s+'
authors_split_regex = '(?i),?\\s+(and|with)\\s+'


#: Use author sort in Tag browser
# ID: categories_use_field_for_author_name
# Set which author field to display in the tags pane (the list of authors, series, publishers etc on the left hand side). The choices are author and author_sort. This tweak affects only what is displayed under the authors category in the tags pane and Content server. Please note that if you set this to author_sort, it is very possible to see duplicate names in the list because although it is guaranteed that author names are unique, there is no such guarantee for author_sort values. Showing duplicates won't break anything, but it could lead to some confusion. When using 'author_sort', the tooltip will show the author's name. Examples:
...
#  publication default if not set: MMM yyyy timestamp default if not set: dd MMM yyyy last_modified_display_format if not set: dd MMM yyyy
...
# Set the list of words that are to be considered 'articles' when computing the title sort strings. The articles differ by language. By default, calibre uses a combination of articles from English and whatever language the calibre user interface is set to. In addition, in some contexts where the book language is available, the language of the book is used. You can change the list of articles for a given language or add a new language by editing per_language_title_sort_articles. To tell calibre to use a language other than the user interface language, set, default_language_for_title_sort. For example, to use German, set it to 'deu'. A value of None means the user interface language is used. The setting title_sort_articles is ignored (present only for legacy reasons).
per_language_title_sort_articles = {'ron': ('Un\\s+', 'O\\s+', 'Ni\xc5\x9fte\\s+'), 'tur': ('Bir\\s+',), 'por': ('A\\s+', 'O\\s+', 'Os\\s+', 'As\\s+', 'Um\\s+', 'Uns\\s+', 'Uma\\s+', 'Umas\\s+'), 'ita': ('Lo\\s+', 'Il\\s+', "L'", 'L\xb4', 'La\\s+', 'Gli\\s+', 'I\\s+', 'Le\\s+', 'Uno\\s+', 'Un\\s+', 'Una\\s+', "Un'", 'Un\xb4', 'Dei\\s+', 'Degli\\s+', 'Delle\\s+', 'Del\\s+', 'Della\\s+', 'Dello\\s+', "Dell'", 'Dell\xb4'), 'nld': ('De\\s+', 'Het\\s+', 'Een\\s+', "'n\\s+", "'s\\s+", 'Ene\\s+', 'Ener\\s+', 'Enes\\s+', 'Den\\s+', 'Der\\s+', 'Des\\s+', "'t\\s+"), 'spa': ('El\\s+', 'La\\s+', 'Lo\\s+', 'Los\\s+', 'Las\\s+', 'Un\\s+', 'Una\\s+', 'Unos\\s+', 'Unas\\s+'), 'fra': ('Le\\s+', 'La\\s+', "L'", u'L\xb4', u'L\u2019', 'Les\\s+', 'Un\\s+', 'Une\\s+', 'Des\\s+', 'De\\s+La\\s+', 'De\\s+', "D'", u'D\xb4', u'L\u2019'), 'ell': ('O\\s+', 'I\\s+', 'To\\s+', 'Ta\\s+', 'Tus\\s+', 'Tis\\s+', "'Enas\\s+", "'Mia\\s+", "'Ena\\s+", "'Enan\\s+"), 'eng': ('A\\s+', 'The\\s+', 'An\\s+'), 'swe': ('En\\s+', 'Ett\\s+', 'Det\\s+', 'Den\\s+', 'De\\s+'), 'deu': ('Der\\s+', 'Die\\s+', 'Das\\s+', 'Den\\s+', 'Ein\\s+', 'Eine\\s+', 'Einen\\s+', 'Dem\\s+', 'Des\\s+', 'Einem\\s+', 'Eines\\s+'), 'epo': ('La\\s+', "L'", 'L\xb4'), 'hun': ('A\\s+', 'Az\\s+', 'Egy\\s+'), 'afr': ("'n\\s+", 'Die\\s+')}


#: Specify a folder calibre should connect to at startup
# ID: auto_connect_to_folder
# Specify a folder that calibre should connect to at startup using connect_to_folder. This must be a full path to the folder. If the folder does not exist when calibre starts, it is ignored. If there are '\' characters in the path (such as in Windows paths), you must double them. Examples:
...
# Set whether to use one or two columns for custom metadata when editing metadata  one book at a time. If True, then the fields are laid out using two columns. If False, one column is used.
...
# The number of seconds to wait before sending emails when using a public email server like gmx/hotmail/gmail. Default is: 5 minutes Setting it to lower may cause the server's SPAM controls to kick in, making email sending fail. Changes will take effect only after a restart of calibre. You can also change the list of hosts that calibre considers to be public relays here. Any relay host ending with one of the suffixes in the list below will be considered a public email server.
public_smtp_relay_delay = 31
...
# Right-clicking the "View" button shows a list of recently viewed books. Control how many should be shown, here.
gui_view_history_size = 15


#: Change the font size of book details in the interface
# ID: change_book_details_font_size_by
# Change the font size at which book details are rendered in the side panel and comments are rendered in the metadata edit dialog. Set it to a positive or negative number to increase or decrease the font size.
change_book_details_font_size_by = 0


#: What format to default to when using the "Unpack book" feature
# ID: default_tweak_format
# The "Unpack book" feature of calibre allows direct editing of a book format. If multiple formats are available, calibre will offer you a choice of formats, defaulting to your preferred output format if it is available. Set this tweak to a specific value of 'EPUB' or 'AZW3' to always default to that format rather than your output format preference. Set to a value of 'remember' to use whichever format you chose last time you used the "Unpack book" feature. Examples:
...
# This means that you can make changes and press Enter and your changes will not be overwritten by a matching completion. However, if you wish to use the completions you will now have to press Tab to select one before pressing Enter. Which technique you prefer will depend on the state of metadata in your library and your personal editing style.
...
# This means that when sorting on text fields like title the text "Book 2" will sort before the text "Book 100". If you want this behavior, set numeric_collation = True note that doing so will cause problems with text that starts with numbers and is a little slower.
numeric_collation = False


#: Sort the list of libraries alphabetically
# ID: many_libraries
# The list of libraries in the Copy to library and Quick switch menus are normally sorted by most used. However, if there are more than a certain number of such libraries, the sorting becomes alphabetic. You can set that number here. The default is ten libraries.
many_libraries = 10
...
# The quality of a thumbnail is largely controlled by the compression quality used when creating it. Set this to a larger number to improve the quality. Note that the thumbnails get much larger with larger compression quality numbers. The value can be between 50 and 99
...
# ID: cover_drop_exclude
# Normally, if you drop any image file in a format known to calibre onto the "Book details" panel, it will be used to set the cover. If you want to store some image types as e-books instead, you can set this tweak. Examples:
#     cover_drop_exclude = {'tiff', 'webp'}
cover_drop_exclude = ()


#: Show the Saved searches box in the Search bar
# ID: show_saved_search_box
# In newer versions of calibre, only a single button that allows you to add a new Saved search is shown in the Search bar. If you would like to have the old Saved searches box with its two buttons back, set this tweak to True.
show_saved_search_box = False


#: Exclude fields when copy/pasting metadata
# ID: exclude_fields_on_paste
# You can ask calibre to not paste some metadata fields when using the Edit metadata->Copy metadata/Paste metadata actions. For example, exclude_fields_on_paste = ['cover', 'timestamp', '#mycolumn'] to prevent pasting of the cover, Date and custom column, mycolumn.
exclude_fields_on_paste = []


#: Skip internet connected check
# ID: skip_network_check
# Skip checking whether the internet is available before downloading news. Useful if for some reason your operating systems network checking facilities are not reliable (for example NetworkManager on Linux).
